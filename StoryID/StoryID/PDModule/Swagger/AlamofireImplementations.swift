...
// MARK: - AlamofireRequestBuilderFactory
...
// MARK: - AlamofireRetrier

public enum AlamofireRetrier {
    public static var interceptor: SIDInterceptor?
}
...
open class AlamofireRequestBuilder<T>: RequestBuilder<T> {
    ...
    open func makeRequest(manager: Session, method: HTTPMethod, encoding: ParameterEncoding, headers: HTTPHeaders) -> DataRequest {
        return manager.request(URLString, method: method, parameters: parameters, encoding: encoding, headers: headers, interceptor: AlamofireRetrier.interceptor)
    }
    ...
    override open func execute(_ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {
        ...
        let xMethod = HTTPMethod(rawValue: method.uppercased())
        let fileKeys = parameters == nil ? [] : parameters!.filter { $1 is NSURL }.map { $0.0 }

        if !fileKeys.isEmpty {
            let uploadRequest = manager.upload(multipartFormData: { mpForm in
            ...
            }, to: URLString, usingThreshold: MultipartFormData.encodingMemoryThreshold, method: xMethod, headers: nil, interceptor: AlamofireRetrier.interceptor)

            uploadRequest.responseJSON { response in
                    switch response.result {
                    case .success:
                        let doneProgress = Progress(totalUnitCount: 100)
                        doneProgress.completedUnitCount = 100
                        self.onProgressReady?(doneProgress)

                        self.processRequest(request: uploadRequest, managerId, completion)
                    case let .failure(afError):
                        completion(nil, ErrorResponse.error(415, nil, afError))
                    }
                }
        } else {
            let request = makeRequest(manager: manager, method: xMethod, encoding: encoding, headers: .init(headers))
    ...
    fileprivate func processRequest(request: DataRequest, _ managerId: String, _ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {
        ...
        switch T.self {
        case is String.Type:
            validatedRequest.responseString { stringDataResponse in
                cleanupRequest()

                switch stringDataResponse.result {
                case .success:
                    completion(Response(response: stringDataResponse.response!,
                                        body: stringDataResponse.value as? T),
                               nil)

                case let .failure(afError):
                    completion(nil, ErrorResponse.error(stringDataResponse.response?.statusCode ?? 500, stringDataResponse.data, afError))
                }
            }
        case is URL.Type:
            validatedRequest.responseData(completionHandler: { dataResponse in
                ...
                do {
                    ...
                    if case .failure = dataResponse.result {
        ...
        case is Void.Type:
            validatedRequest.responseData(completionHandler: { voidResponse in
                ...
                case .success:
                    completion(Response(response: voidResponse.response!, body: nil), nil)

                case let .failure(afError):
                    completion(nil, ErrorResponse.error(voidResponse.response?.statusCode ?? 500, voidResponse.data, afError))
                }
            })
        default:
            validatedRequest.responseData(completionHandler: { dataResponse in
                ...
                case let .failure(afError):
                    completion(nil, ErrorResponse.error(dataResponse.response?.statusCode ?? 500, dataResponse.data, afError))
                }
            })
        }
    }
...
// MARK: - AlamofireDecodableRequestBuilderError
...
// MARK: - AlamofireDecodableRequestBuilder
    ...
    override fileprivate func processRequest(request: DataRequest, _ managerId: String, _ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {
        ...
        case is String.Type:
            validatedRequest.responseString(completionHandler: { stringResponse in
                ...
                case .success:
                    completion(Response(response: stringResponse.response!, body: (stringResponse.value ?? "") as! T), nil)
                case let .failure(afError):
                    completion(nil, ErrorResponse.error(stringResponse.response?.statusCode ?? 500, stringResponse.data, afError))
                }
            })
        case is Void.Type:
            validatedRequest.responseData(completionHandler: { voidResponse in
        ...
        default:
            validatedRequest.responseData(completionHandler: { (dataResponse: AFDataResponse<Data>) in
                ...
                if case let .failure(afError) = dataResponse.result {
                    completion(nil, ErrorResponse.error(dataResponse.response?.statusCode ?? 500, dataResponse.data, afError))
                    return
                }
